...
class HrAttendance(models.Model):
    ...
    def _create_resume_from_no_imputed_holidays(self, leave, employee, date):
        calendar = employee._get_employee_calendar(date)
        ...
        for record in self:
            ...
            if record.leave_id:
                ...
                if record.theoretical_time:
                    ...
                    work, hour_from, hour_to = employee._get_day_work_time(
                        calendar, record.date)
                    record.holiday_hours = work

    @api.depends('theoretical_time_final', 'work_time', 'bonus_hours',
                 'rest_exceeded')
