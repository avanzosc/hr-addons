...
class TestHrTimesheetUsability(common.SavepointCase):
    ...
    def setUpClass(cls):
        ...
        cls.analytic1 = cls.env["account.analytic.line"].create(cls.analytic_vals1)
        cls.task_vals1 = {
            "name": "abcd",
            "user_id": 2,
            "project_id": cls.env["project.project"].search([], limit=1).id,
    ...
    def test_hr_timesheet_usability(self):
        ...
        self.assertEqual(self.analytic1.time_stop, round(hours + minutes / 60, 14))
        self.assertEqual(self.task1.show_init_task, True)
        self.task1.action_button_initiate_task()
        self.assertEqual(self.task1.user_id.id, self.task1.timesheet_ids[0].user_id.id)
        self.assertEqual(
            self.task1.employee_id.id, self.task1.timesheet_ids[0].employee_id.id
        )
        self.task1._compute_show_init_task()
        self.assertEqual(self.task1.show_init_task, False)
        self.task1.timesheet_ids[0].write({"time_start": 6.0, "time_stop": 6.0})
