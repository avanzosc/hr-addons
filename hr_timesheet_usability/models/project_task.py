...
class ProjectTask(models.Model):
    ...
    def action_button_initiate_task(self):
        ...
        if self.user_id:
            ...
            if not self.employee_id:
                raise UserError(
                    _("Employee not found for user: {}").format(self.user_id.name)
                )
            initiate_timesheet_vals.update(
                {"user_id": self.user_id.id, "employee_id": self.employee_id.id}
            )
        return self.env["account.analytic.line"].create(initiate_timesheet_vals)
    ...
    def action_button_end_task(self):
        ...
        for line in lines:
            ...
            date_end = date_end.replace(tzinfo=pytz.timezone("UTC")).astimezone(
                timezone
            )
            ...
            if stop1 > start:
                ...
                vals = {"time_stop": round(stop, 14), "unit_amount": unit_amount}
                line.write(vals)
