...
class HrAttendance(models.Model):
        ...
        while min_date <= max_date:
            ...
            if not festive:
                ...
                if not resume:
                    ...
                    calendar = emp._get_employee_calendar(min_date)
                    work_time, hour_from, hour_to = emp._get_day_info(
                        calendar, min_date)
                    if work_time != 0.0:
                        vals = {
                            'employee_id': min_check_in[0].employee_id.id,
                            'date': min_date,
    ...
    def _impute_employee_attendances_day(self):
        ...
        if not resume.leave_id and resume.imputable_time:
            calendar = resume.employee_id._get_employee_calendar(resume.date)
            employee = resume.employee_id
            work_time, hour_from, hour_to = employee._get_day_work_time(
                calendar, resume.date)
            error = False
